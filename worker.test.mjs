...
test('typing unlocks at 30 / 1.3 seconds, never before',()=>{for(const elapsed of [0,18000,23000,30000/1.3-.001])assert.equal(IntroTiming.canType(elapsed),false);assert.equal(IntroTiming.canType(30000/1.3),true);assert.equal(IntroTiming.canType(90000),true);});
test('stars begin forming text at 18 / 1.3 seconds and finish at 28 / 1.3',()=>{assert.equal(IntroTiming.morph(17000/1.3),0);assert.equal(IntroTiming.morph(23000/1.3),.5);assert.equal(IntroTiming.morph(28000/1.3),1);});
