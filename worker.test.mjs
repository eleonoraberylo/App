...
test('typing unlocks at 80 seconds, never before',()=>{for(const elapsed of [0,52000,70000,79999])assert.equal(IntroTiming.canType(elapsed),false);assert.equal(IntroTiming.canType(80000),true);assert.equal(IntroTiming.canType(90000),true);});
test('stars begin forming text at 52 seconds and finish at 78',()=>{assert.equal(IntroTiming.morph(51000),0);assert.equal(IntroTiming.morph(65000),.5);assert.equal(IntroTiming.morph(78000),1);});
