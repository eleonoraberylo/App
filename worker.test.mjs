...
test('typing unlocks at 30 seconds, never before',()=>{for(const elapsed of [0,18000,28000,29999])assert.equal(IntroTiming.canType(elapsed),false);assert.equal(IntroTiming.canType(30000),true);assert.equal(IntroTiming.canType(90000),true);});
test('stars begin forming text at 18 seconds and finish at 28',()=>{assert.equal(IntroTiming.morph(17000),0);assert.equal(IntroTiming.morph(23000),.5);assert.equal(IntroTiming.morph(28000),1);});
