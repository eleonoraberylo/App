...
function colorsFor(name){const palette={peaceful:'#b4d6c6',peace:'#b4d6c6',calm:'#b4d6c6',relaxed:'#b4d6c6',stressed:'#ded2a9',stress:'#ded2a9',anxious:'#ded2a9',worried:'#ded2a9',furious:'#d6afb8',angry:'#d6afb8',anger:'#d6afb8',annoyed:'#dfbfa9',frustrated:'#dfbfa9',sad:'#b2c9df',lonely:'#b2c9df',hopeful:'#c6badf',happy:'#c6badf',joyful:'#c6badf',affectionate:'#d9b9cd',loved:'#d9b9cd',grateful:'#d9b9cd'};const words=name.toLowerCase().match(/[a-z']+/g)||[];const colors=words.flatMap((word,i)=>palette[word]&&!words.slice(Math.max(0,i-2),i).some(w=>['not','never',"don't",'less'].includes(w))?[palette[word]]:[]);return [...new Set(colors)].slice(0,3).length?[...new Set(colors)].slice(0,3):['#92a9ca','#b6a1c7'];}
