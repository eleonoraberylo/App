...
function laneY(i,x=width*.5){
 ...
 let shock=0;const age=clock-pulse;
 if(!paused&&i===activeLane&&pulse>=0&&age<1600){
  const center=((age/650)%1)*1.5-.25,z=(u-center)*45;
  const shape=-.28*Math.exp(-Math.pow(z+1.4,2)*4)+Math.exp(-z*z*6)-.62*Math.exp(-Math.pow(z-.8,2)*7);
  shock=shape*Math.min(height*.1,75)*Math.pow(1-age/1600,.6);
 }
 return height*laneHeights[i]+drift+swell+buzz+shock-(camera.y-.5)*(5+i);
}
...
function draw(dt){
 ...
 for(let i=0;i<14;i++)currentColors[i]=blendColor(previousColors[i]||currentColors[i],palette[i%palette.length],paused?1:Math.min(1,age/1400));
 ...
 for(let i=0;i<14;i++){
  ...
  ctx.beginPath();for(let px=-30;px<=width+30;px+=5){const py=laneY(i,px);if(px===-30)ctx.moveTo(px,py);else ctx.lineTo(px,py);}
