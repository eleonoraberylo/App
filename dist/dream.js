...
function makeTargets(){
 ...
 g.letterSpacing=style.letterSpacing;g.fillText(style.textTransform==='uppercase'?label.textContent.toUpperCase():label.textContent,r.left,r.top);
 const i=input.getBoundingClientRect(),is=getComputedStyle(input);g.font=is.fontSize+' '+is.fontFamily;g.letterSpacing=is.letterSpacing;g.fillText('…',i.left,i.top+5);
...
function draw(dt){
 ...
 const intro=!window.introReady,seconds=elapsed*IntroTiming.speed/1000;
 const flight=paused?0:Math.pow(Math.max(0,Math.sin(Math.PI*Math.min(1,seconds/19))),3);
 const handoff=window.introReady?Math.max(0,1-(elapsed-IntroTiming.durationMs)/(1400/IntroTiming.speed)):1;
 ...
 for(let j=0;j<points.length;j++){
  ...
  if((intro||handoff>0)&&target&&!paused){
   ...
   alpha=(alpha*(1-smooth)+smooth*.98)*handoff;
  ...
  ctx.globalAlpha=Math.min(1,alpha);ctx.fillStyle=kick>0?tint:'#f3fff8';
  const size=intro&&target?.9:.85+Math.pow(p.s,6)*.8;
