...
function makeTargets(){
 ...
 g.font=s.fontWeight+' '+s.fontSize+' '+s.fontFamily;g.letterSpacing=s.letterSpacing;g.textBaseline='alphabetic';g.fillStyle='#fff';
...
function draw(dt){
 ...
 for(let j=0;j<points.length;j++){
  ...
  alpha*=1-smooth*.35;
  if(forming)alpha=(alpha*(1-gather)+.64*gather)*(1-Math.min(1,Math.max(0,(elapsed-13500)/1500)));
  if(forming&&progress>0&&progress<1&&Number.isFinite(p.px)){
   ctx.globalAlpha=alpha*.32;ctx.strokeStyle='#9ba8a1';ctx.lineWidth=.6;ctx.beginPath();ctx.moveTo(p.px,p.py);ctx.lineTo(x,y);ctx.stroke();
  }
  p.px=x;p.py=y;
  ...
  ctx.globalAlpha=Math.min(1,alpha);ctx.fillStyle=forming&&progress>.3?'#adb5b0':kick>0?tint:'#f3fff8';
